...
class StaffService extends FuseUtils.EventEmitter {
    ...
    assignServiceToStaff = postData => {
        return new Promise((resolve, reject) => {
            axios.post(`/api/UserServices/{SESSIONCODE}`, postData)
                .then(response => {
                    if (response.data) {
                        resolve(response.data);
                    }
                    else {
                        reject('Unable to assign service to staff');
                    }
                })
                .catch(error => {
                    // this.logout();
